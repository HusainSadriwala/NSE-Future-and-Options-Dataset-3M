...
The source dataset is a flat CSV containing high-volume NSE Futures & Options data with repeated attributes such as symbol, expiry date, and strike price. To eliminate redundancy and improve scalability, the schema is normalized to Third Normal Form (3NF) by separating exchanges, instruments, contract expiries, and trade metrics into distinct tables.

A star schema was intentionally avoided because this system is optimized for frequent ingestion and time-series analytics rather than BI-style aggregations. Normalization reduces storage duplication, improves write performance, and simplifies future data ingestion.
